  ...
  useEffect(() => {
    fetch('/daily_content.json')
      ...
      .then(data => {
        ...
        const found = data.find(item => item.date === today) || data[0]
        setTodayContent(found)
        setLoading(false)
      })
      .catch(err => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-warm">
        <p className="text-brand-soft italic">Loading your daily dose of calm...</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-2xl mx-auto font-sans text-brand-text">
      <header className="text-center mb-12">
        ...
        <nav className="mt-8 flex justify-center gap-6 text-sm font-medium uppercase tracking-widest text-brand-soft">
          <button 
            ...
            className={`pb-1 border-b-2 transition-colors ${view === 'today' ? 'border-brand-soft text-brand-text' : 'border-transparent hover:text-brand-text'}`}
          >
            Today
          </button>
          <button 
            onClick={() => setView('archive')}
            className={`pb-1 border-b-2 transition-colors ${view === 'archive' ? 'border-brand-soft text-brand-text' : 'border-transparent hover:text-brand-text'}`}
          >
            Archive
          </button>
        </nav>
      ...
      <main>
        {view === 'today' ? (
          todayContent ? (
            <div className="bg-white rounded-3xl shadow-sm p-8 md:p-12 mb-8 border border-brand-accent/20 transition-all hover:shadow-md">
        ...
        <section className="mt-16 border-t border-brand-accent/30 pt-12 text-center">
          ...
          <div className="bg-brand-accent/10 p-8 rounded-3xl">
            ...
            <p className="text-brand-soft mb-6 max-w-md mx-auto">
              Unlock weekly deep reflections, audio versions, and printable activity cards for just $4.99/month.
            </p>
            <button className="bg-brand-soft text-brand-warm px-10 py-4 rounded-full font-semibold hover:bg-brand-text transition-all transform hover:scale-105 shadow-sm">
              Try Raising Today Premium
            </button>
          </div>
        </section>
      </main>

      <footer className="mt-20 text-center text-xs text-brand-soft/60 pb-8">
        &copy; 2026 Raising Today. All rights reserved.
      </footer>
    </div>
  )
}
